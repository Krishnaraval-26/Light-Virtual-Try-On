...
def generate_room_with_light(
    ...
    # Build parts: text + images as inline_data bytes
    parts: list[gt.Part] = [gt.Part(text=user_prompt)]
    ...
    parts.append(
        gt.Part(
            inline_data=gt.Blob(
                mime_type="image/png",
                data=pil_to_png_bytes(room_img),
            )
        )
    )

    # Light images
    for li in light_images:
        parts.append(
            gt.Part(
                inline_data=gt.Blob(
                    mime_type="image/png",
                    data=pil_to_png_bytes(li),
                )
            )
        )
